...
(define (my-vector-append v1 v2)
  (racket-vector-append v1 v2)) ; Use renamed built-in vector-append
    ...
    (cond 
      ...
      (else 
       (let* ((new-history (update-history history user-response))
              (reply-text (reply-v2 strategies user-response new-history)))
         (print reply-text)
...
(define (generate-keyword-reply user-response history)
  (let* ((keyword (select-keyword user-response))
...
(define (generate-reply strategy user-response history)
  (case strategy
    ((0) (hedge-answer user-response history))
    ((1) (qualifier-answer user-response history))
    ((2) (history-answer user-response history))
    ((3) (generate-keyword-reply user-response history))))
...
(define (reply-v2 strategies user-response history)
  (let* ((applicable (filter-applicable-strategies strategies user-response history))
         (selected (pick-random-vector-with-weight applicable strategy-weight)))
    ((strategy-body selected) user-response history)))
...
(define (test-generate-keyword-reply)
  (set! mock-random-values '(0 0))
  (assert-equal "generate-keyword-reply-1"
                (generate-keyword-reply '(i feel depressed about scheme) (vector))
                '(when you feel depressed go out for ice cream))
  (set! mock-random-values '(1 1))
...
(define (test-hedge-answer)
  ...
  (assert-equal "hedge-answer-1" (hedge-answer '() (vector)) '(please go on))
  (assert-equal "hedge-answer-2" (hedge-answer '() (vector)) '(tell me more))
  (assert-equal "hedge-answer-3" (hedge-answer '() (vector)) '(interesting)))

(define (test-qualifier-answer)
  (set! mock-random-values '(0))
  (assert-equal "qualifier-answer-1"
                (qualifier-answer '(i feel sad) (vector))
...
(define (test-generate-reply)
  (set! mock-random-values '(0 0 0 0 0))
